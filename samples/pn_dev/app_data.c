...
uint8_t * app_data_get_input_data ( uint16_t slot_nbr, uint16_t subslot_nbr,
    uint32_t submodule_id, bool button_pressed, uint16_t * size, 
    uint8_t * iops) {
    ...
    printf ( "%s: seq:%lu,  slot_nbr:%x, sub_nbr: %x, sub_id:%x\n",
            __FUNCTION__, seq++, slot_nbr, subslot_nbr, submodule_id);
